...
export const SOUTH = [0, 0, -1]; // 南極＝|1⟩。S はゲート名とまぎらわしいので使わない
...
export const PAGE_COPY = {
  ...
  cover: {
    ...
    goalLabel: '今日のゴール',
    goal: 'ブロックをならべると「消える／変身する」の法則を見つけよう',
    heroName: 'キュービット',
    heroNameSuffix: '君',
  ...
  story: {
    ...
    figureXBlock: 'Xブロック',
    step4: '<Gate name="X" /> は、<b>6つある ブロックのなかまの1つ</b>。やじるしが <b>どこを向いていても</b> <b style="color:#f59e0b">xじく</b>を中心に <b>半周</b> まわすよ。まずは <b>北極</b>を指しているキュービット君で見てみよう。',
    step4b: 'つぎは <b>ヘルシンキ（フィンランド）</b> を指しているキュービット君で見てみよう。',
    // 北極とヘルシンキ、2つの例を見たあとの結論。ヘルシンキの図の下（⑤の直前）に置く。
    step4bResult: '出発点はちがっても、どちらも同じ <b style="color:#f59e0b">xじく</b>のまわりを 半周。<b>2回わたすと 半周＋半周＝1周</b>で、やじるしは <b>元に戻った！</b>',
    ...
    step5: '元に戻った＝ <b><Gate name="X" />を2回わたす意味がない</b>。キュービット君には なんの えいきょうもない！ だから <b><GatePair name="X" /></b> のならびがあったら <b>消してもいい</b>！',
    match: 'マッチして消える！',
  ...
  intro: {
    ...
    howto: 'ブロックをキュービット君にわたすと、キュービット君の<b>やじるし</b>が<b>くるっと回転</b>するよ。<b>まわるじく</b>と<b>まわる量</b>のちがいを、よく見くらべよう。',
  },
  pairs: {
    sub: '③ 2つのブロックでマッチ ・ p.4',
  ...
  about: {
    ...
    // 各行列にそえる回転軸と回転角。H の軸は x+z 方向（回転軸は向きだけが意味を持つので正規化は書かない）。
    axisNotes: { X: 'x軸 180°', Z: 'z軸 180°', S: 'z軸 90°', H: 'x+z軸 180°' },
    // 「プリントの図」ではどの図か伝わらないので、子ども向けページの言い方（ブロックをわたす・赤いやじるし）と
    // 球の図が出るページ範囲（p.7 の SWAP には球が出ないので p.2〜p.6）を明示して対応づける。
    applyText: '状態ベクトルにゲートを適用する＝行列とベクトルの掛け算です。本教材で「キュービット君にブロックをわたす」と赤い矢印の向きが変わる図（p.2〜p.6）は、この掛け算の結果を描いたものです。',
    eqNote1: '→ キュービット君が 北極→南極 に反転（p.2 の X）。',
    // 例2の式にはさむ日本語。行列そのものは描画側で組む。
    eqWords: { soThen: 'つまり', identity: '単位行列' },
    // 式で「= I 単位行列」まで見せたうえで、「消える」がその I になることだと注記で結ぶ。
    eqNote2: '→ X を2回で元通り。「マッチして消える」とは、ブロックの並びが I になることです（p.4 の X²）。',
    eqNote3: '→ S を2回で Z と同じ（p.4 の S²→Z）。T は z軸 45°（位相 e<sup>iπ/4</sup>）で、2回で S になります。',
    aimTitle: 'この教材のねらい',
