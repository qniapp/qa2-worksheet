...
const stateFigure = (size, state, caption, ghostState = null, axisHighlight = null, pathAngle = null) =>
  ...
  <div class="step"><div class="num">4</div>
    <div class="stepbody">
      ...
      <div class="strip">
        ...
        ${storyAction('X')}
        <figure><div class="figlabel">${furi('② X後：南極')}</div>${globe({ size: 124, skin: 'earth', state: [0,0,-1], face: true, poleLabels: false, ghostState: N, axisHighlight: GATES.X.axis, pathAxis: GATES.X.axis, pathAngle: 180 })}<figcaption>${furi('南極へ')}</figcaption></figure>
        ${storyAction('X')}
        <figure><div class="figlabel">${furi('③ 2回目：北極')}</div>${globe({ size: 124, skin: 'earth', state: N, face: true, poleLabels: false, ghostState: [0,0,-1], axisHighlight: GATES.X.axis, pathAxis: GATES.X.axis, pathAngle: 180 })}<figcaption>${furi('元に戻った！')}</figcaption></figure>
      </div>
    </div>
  </div>
...
const triplesPage = (sub, heading, lead, rows, n, memo) => `<div class="page triples ${n === 6 ? 'tallrows' : ''}">
  ...
  ${memo ? `<div class="freewrite guided triplememo" style="height:${memo}px"><div class="fwh">✏️ ${furi('よそう・きづいたこと')}</div><div class="promptlines"><span>Hではさむと ${inlineGate('X')} は ＿＿ になる</span><span>Hではさむと ${inlineGate('Z')} は ＿＿ になる</span><span>Hではさむと ${inlineGate('Y')} は ＿＿ になる</span><span class="freeprompt">そのほか きづいたこと：</span></div></div>` : ''}
  ${footer(n)}
</div>`;
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .pleft { width: 30%; border-right: 2px dashed #e2e8f0; padding-right: 10px; } .tleft { width: 30%; }
  .pairs .pleft { width: 25%; }
