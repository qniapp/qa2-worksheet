...
- `docs/agents/matt-workflow.md` — Matt skills 前提の標準開発フロー、GBrain 運用
...
4. **Matt skills 前提の開発運用に寄せること**
   - 新規実装は issue ごとに fresh `/implement` で進め、完了後に `/code-review` を通す。
   - `/to-tickets` 由来で `ready-for-agent` が付いた issue は、原則として追加の `/triage` を挟まない。
   - 詳細な issue / PR 操作や skill の使い分けは `docs/agents/` に置く。

5. **不可逆・広範囲な操作は人間の明示指示を待つこと**
...
`content/worksheet-content.mjs`、`src/worksheet/`、`build.mjs` など、生成される PDF / HTML の見た目や読みやすさに影響するファイルを変更した場合は、必ず次を行ってください。
...
- `docs/agents/issue-tracker.md` — GitHub issue / PR 操作の最小手順
- `docs/agents/matt-workflow.md` — Matt skills、GBrain 運用
