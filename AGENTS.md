...
6. すべてのページについて `interface-craft` の Design Critique スキルでレビューする。
7. コメントが出た場合は修正し、再ビルド・再レビューを繰り返す。
8. PR 本文に、変更の主対象ページと全ページ確認画像へのリンク / 埋め込みを置く。
9. 全ページでコメントがなくなり PASS になり、PR 本文から人間が画像確認できるまで完了扱いにしない。
